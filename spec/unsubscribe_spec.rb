...
describe Mailgunna::Unsubscribe do
  ...
  describe "list unsubscribes" do
    it "should make a GET request with the right params" do
      ...
      unsubscribes_url = @mailgunna.unsubscribes(@sample[:domain]).send(:unsubscribe_url)
      Mailgunna.should_receive(:submit).
        with(:get, unsubscribes_url, {}).
        and_return(sample_response)

      @mailgunna.unsubscribes(@sample[:domain]).list
    end
  end
  ...
  describe "find unsubscribe" do
    it "should make a GET request with the right params to find given email address" do
      sample_response = "{\"items\": [{\"size_bytes\": 0,  \"mailbox\": \"postmaster@bsample.mailgunna.org\" }  ]}"
      unsubscribes_url = @mailgunna.unsubscribes(@sample[:domain]).send(:unsubscribe_url, @sample[:email])

      Mailgunna.should_receive(:submit)
        .with(:get, unsubscribes_url)
        .and_return(sample_response)

      @mailgunna.unsubscribes(@sample[:domain]).find(@sample[:email])
    end
  end
  ...
  describe "delete unsubscribe" do
    it "should make a DELETE request with correct params to remove a given email address" do
      ...
      @mailgunna.unsubscribes(@sample[:domain]).remove(@sample[:email])
    end
  end
  ...
  describe "add unsubscribe" do
    context "to tag" do
      it "should make a POST request with correct params to add a given email address to unsubscribe from a tag" do
        ...
        Mailgunna.should_receive(:submit)
          .with(:post, "#{@mailgunna.unsubscribes(@sample[:domain]).send(:unsubscribe_url)}",{:address=>@sample[:email], :tag=>@sample[:tag]})
          .and_return(response_message)

        @mailgunna.unsubscribes(@sample[:domain]).add(@sample[:email], @sample[:tag])
      end
    end
    ...
    context "on all" do
      it "should make a POST request with correct params to add a given email address to unsubscribe from all tags" do
        ...
        @mailgunna.unsubscribes(@sample[:domain]).add(@sample[:email])
      end
    end
  end
